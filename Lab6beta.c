...
//unsigned long *input_matrix[N];

//unsigned long long threadless[N][N], threaded[N][N], squared_threads[N][N];
//unsigned long long *threadless[N], *threaded[N], *squared_threads[N];
	...
	unsigned long get_random();
{
	...
	//get user input
	printf("please enter a width for the matrix: \n");
	scanf("%u", &N);
	char input[2];
	...
	for (i = 0; i < N; i++)
	{
		input_matrix[i] = (unsigned long *)malloc(N * sizeof(unsigned long));
		threadless[i] = (unsigned long long *)malloc(N * sizeof(unsigned long long));
    	threaded[i] = (unsigned long long*)malloc(N * sizeof(unsigned long long));
    	squared_threads[i] = (unsigned long long *)malloc(N * sizeof(unsigned long long));
    
    }


    //check to see if user wants to use an identity matrix
	if (*input == 'n')
	{
		//fill matrix
		for (i = 0; i < N; i++)
...
void *threaded_method(void *param)
{
	for (int i = 0; i < N; i++)
	{
...
void *squared_thread_method(void *param)
{
